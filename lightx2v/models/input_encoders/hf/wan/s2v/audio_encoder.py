...
class AudioEncoder:
    ...
    def get_audio_embed_bucket(self, audio_embed, stride=2, batch_frames=12, m=2):
        ...
        for bi in batch_idx:
            if bi < audio_frame_num:
                ...
                frame_audio_embed = (
                    torch.zeros([audio_dim * (2 * m + 1)], device=audio_embed.device) if not return_all_layers else torch.zeros([num_layers, audio_dim * (2 * m + 1)], device=audio_embed.device)
                )
    ...
    def get_audio_embed_bucket_fps(self, audio_embed, fps=16, batch_frames=81, m=0):
        ...
        batch_idx = get_sample_indices(original_fps=self.video_rate, total_frames=audio_frame_num + padd_audio_num, target_fps=fps, num_sample=bucket_num, fixed_start=0)
        batch_audio_eb = []
