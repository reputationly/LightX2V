...
class VideoAutoencoderKLWrapper(VideoAutoencoderKL):
    ...
    def vae_encode(self, samples: List[Tensor]) -> List[Tensor]:
        ...
        if len(samples) > 0:
            ...
            scale = torch.tensor(scale, device=AI_DEVICE, dtype=GET_DTYPE())
            shift = torch.tensor(shift, device=AI_DEVICE, dtype=GET_DTYPE())
    ...
    def vae_decode(self, latents: List[Tensor]) -> List[Tensor]:
        ...
        if len(latents) > 0:
            ...
            for latent in latents:
                latent = latent.to(AI_DEVICE, dtype=GET_DTYPE())
...
def attn_video_vae_v3_s8_c16_t4_inflation_sd3_init(
    *,
    ...
    if weights_path is not None:
        state = torch.load(
            str(weights_path),
            map_location=weights_map_location,
            mmap=weights_mmap,
        )
