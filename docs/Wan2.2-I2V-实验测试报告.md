...
### "720p" 是面积预算,不是固定 1280×720

`max_area = target_height × target_width = 720×1280 = 921,600` 是**像素总量预算**,实际宽高**按输入图比例缩放到这个面积**(`wan_runner.py:510-511`)。所以输出形状跟着图走,但总像素都 ≈92万(0.92MP,720p 级清晰度):

| 输入图 | 宽高比 | 输出 | 总像素 |
|---|---|---|---|
| 猫(竖) | 1.33 | 832×1104 | 918,528 |
| 蓝鸟(方) | 1.0 | 960×960 | 921,600 |
| 16:9 横图 | 1.78 | ~1280×720 | 921,600 |

→ 960×960 不是"分辨率低",和 832×1104 同像素量、同清晰度,只是画幅形状不同。**要什么画幅(横/竖/方)就喂什么比例的首帧图。**
...
脚本封装:`TASK=flf2v LAST_FRAME=<尾帧> bash test_wan_i2v.sh`。分辨率/显存/帧数约束与 i2v 相同。

### flf2v 时长上限 ≈ 8s(画质受限,与 i2v 不同)⚠️

实测:flf2v **≤8s 正常**;**≥10s 出现首帧在 ~8s 重现 + 循环**(首→尾过渡走完一遍又重头,结尾才命中尾帧)。
- 原因:模型的"首→尾形变弧"只在蒸馏训练长度(~81帧)上学过,**末尾被钉死在尾帧**;拉长到 161 帧它不会把形变放慢,只能把 ~8s 的弧**重复填时间** → 首帧重现。
- 对比 i2v:无末尾锚点,自由续动,能优雅外推到 15s+。
- **结论**:flf2v 实用 **≤8s(稳妥 ≤121帧/7.5s)**;显存允许更长但过渡会循环崩。**i2v 时长瓶颈是显存,flf2v 是画质。**

### flf2v 算力开销 = i2v(同分辨率下几乎白嫖)

720p 压测(蓝鸟 960×960,面积 921,600,与 i2v 猫 832×1104 的 918,528 仅差 0.3%,可公平对比):

| 帧数 | 时长 | i2v 生成/GPU | flf2v 生成/GPU | 差距 |
|---|---|---|---|---|
| 81 | 5s | 87s / 35.3G | 89s / 35.3G | ~0% |
| 161 | 10s | 193s / 40.3G | 195s / 39.8G | ~1% |

**同分辨率下 flf2v 与 i2v 的生成耗时/GPU 峰值几乎完全相同**(差 1~2s)。flf2v 只多了 VAE encode 对尾帧的一次 interpolate,DiT 主体(占 ~99% 时间)latent shape 相同 → 开销可忽略。
> 注:480p 时"flf2v 比 i2v 快一倍"是假象 —— 蓝鸟方图 480×480 比猫竖图 480×832 小 1.73×,是分辨率差异不是任务差异。
> CPU:容器内存 64~75G(随帧数/分辨率涨,远低于 256G);CPU% 平时 ~400%(4 rank),VAE 解码瞬间飙到 1800%(多核爆发,极短)。
...
| "720p" 含义 | 面积预算 ~92万像素,宽高跟输入图比例(竖832×1104/方960×960/横~1280×720) |
| i2v 长视频画质 | ✅ 480p 15s / 720p 10s 肉眼正常,时长上限 = 显存上限 |
| 首尾帧 flf2v | `task=flf2v` + `last_frame_path`,同权重;**算力 ≈ i2v**(同分辨率差 1~2s) |
| flf2v 时长上限 | ⚠️ **画质受限 ~8s**(>8s 首帧在 8s 重现+循环),实用 ≤121帧/7.5s |
