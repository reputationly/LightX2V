#!/usr/bin/env bash
# VACE + RIFE 插帧验证: 与首片同 seed/prompt/参考图, 唯一差异=挂 RIFE 16→32fps
# 用法: tmux new -s vace5 -d 'bash /data/smoke/vace_rife.sh'
set -u
LX_IMG=crpi-xzr81d0490mc3794.cn-shanghai.personal.cr.aliyuncs.com/reputationly/lightx2v:arm64-a100-latest
exec > >(tee -a /data/outputs/vace_rife.log) 2>&1
docker run --rm --gpus all --memory=240g --memory-swap=240g -e CUDA_VISIBLE_DEVICES=0 \
  ...
  --config_json /data/smoke/wan22_moe_vace_a100_int8_rife.json \
  --prompt "一位女子在海边漫步，长发随风飘动，阳光洒在海面上，电影感十足" \
  --src_ref_images /opt/LightX2V/assets/inputs/imgs/girl.png \
  --save_result_path /data/outputs/vace_r2v_rife.mp4 --seed 42
