#!/usr/bin/env bash
# VACE + Lightning 4步蒸馏LoRA 实验: 与首片同 seed/prompt/参考图, 40步→4步
# 前置: bash /data/smoke/download_vace_lora.sh 已完成(x2v格式LoRA已就位)
# 用法: tmux new -s vace_lora -d 'bash /data/smoke/vace_lora.sh'
set -u
